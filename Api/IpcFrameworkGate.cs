...
/// <list type="number">
/// <item><b>讀遊戲的原生記憶體</b>——<c>ObjectTable.SearchById</c> 與
/// <c>MarkerUtility.GetObjIndexById</c> 的物件表走訪（後者逐格讀 <c>ObjectTable[i]</c>）。
/// 那些物件由遊戲主執行緒每幀重建，讀到一半被換掉就是 AccessViolationException，
/// 而 AVE 在 .NET Core 是 corrupted-state exception ——<b><c>try</c>/<c>catch</c> 攔不到</b>，
/// 整個遊戲直接崩掉。</item>
/// <item><b>寫使用者設定裡的裸字典</b>——<c>Configuration.SourceConfigs</c> 是裸
/// <c>Dictionary&lt;string, SourceData&gt;</c>，<c>RegisterOrUpdateSource</c> 對它
/// <c>TryAdd</c>，而 framework 執行緒每幀在 <c>FinderService.LookFor</c> 裡
/// <c>TryGetValue</c> 同一張表。並行插入弄壞的<b>不是「慢一拍」而是那張表本身</b>。</item>
/// </list>
/// <para>
/// 🔑 所以<b>每一個</b>對外端點都把整段工作交回主執行緒執行。交回去的是<b>整個方法體</b>，
/// 不是只有第一行檢查 —— 這樣連下游的 helper（前置檢查、物件表查詢、共用集合的讀寫）
/// 也一起被覆蓋，不必逐一追。
/// </para>
/// <para>
...
/// </para>
/// <para>
/// 📌 形狀對齊 <c>TCToolbox/TCToolbox/Core/IpcFrameworkGate.cs</c>（同一份閘門的移植）。
/// </para>
/// </remarks>
internal static class IpcFrameworkGate
{
    ...
    {
        if (ServiceManager.Framework.IsInFrameworkUpdateThread) return body();

        if (IsUnloading(endpoint)) return unavailable;

        var state = StatePending;
        var task = ServiceManager.Framework.RunOnFrameworkThread(() =>
        {
            // 呼叫端已經逾時走人了就什麼都不做。
    ...
    {
        if (ServiceManager.Framework.IsInFrameworkUpdateThread)
        {
            body();
            return;
        }
    ...
    {
        if (!ServiceManager.Framework.IsFrameworkUnloading || ServiceManager.Framework.IsInFrameworkUpdateThread) return false;
        if (ShouldLogTimeout(endpoint + "/卸載期"))
        {
            ServiceManager.Log.Information($"[MiniMappingway IPC 閘門] {endpoint} 在 Dalamud 卸載期從別的執行緒被呼叫，已回傳「不可用」值。卸載期的 RunOnFrameworkThread 會就地在呼叫端執行緒執行，閘門保護不了原生記憶體存取；此時功能失效可以接受，崩潰不行。");
        }
        return true;
    }
    ...
    /// </summary>
    /// <remarks>
    /// 🔴 這裡刻意<b>自帶字典＋自己的鎖</b>，不共用外掛裡任何別的節流表：那些表多半是
    /// <b>只在主執行緒使用</b>的裸 <c>Dictionary</c>，而這條路徑跑在呼叫端的執行緒上，
    /// 並行插入弄壞的是整張表 —— 連帶弄壞外掛裡所有模組的節流。
    ...
    private static void ReportTimeout(string endpoint, bool abandoned)
    {
        ...
        ServiceManager.Log.Information(
            $"[MiniMappingway IPC 閘門] {endpoint} 等待遊戲主執行緒超過 {TimeoutMs} 毫秒，已回傳「不可用」值。{outcome}。" +
